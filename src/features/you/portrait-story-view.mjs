...
export function portraitInterpretationsView(p, s) {
  ...
  return `<section class="portrait-interpretations" data-portrait-story="${esc(m.member)}"><header class="pe-section-heading"><span class="eyebrow">THE PATTERNS BENEATH THE PORTRAIT</span><h3>How we read the picture.</h3></header><div class="pe-patterns">${m.cards.map((c, i) => `<button class="pe-pattern" data-action="portrait-signal:${c.id}" data-support-topic="${c.id}" style="${style(c)}"><span class="pe-pattern-kicker"><span>${String(i + 1).padStart(2, '0')} · ${esc(c.label)}</span>${c.trait ? traitGlyph(c.trait) : icon('spark')}</span><h3>${esc(c.title)}</h3><p>${esc(c.note || c.meaning)}</p><span class="pe-pattern-link">${esc(c.note ? 'Your perspective added' : 'Explore this pattern')}${icon('arrow')}</span></button>`).join('')}</div>${m.own && p.ui.portraitNotes?.overall ? `<section class="ps-own-perspective"><span class="eyebrow">IN YOUR OWN WORDS</span><blockquote>${esc(p.ui.portraitNotes.overall.text)}</blockquote>${button('Edit your perspective', 'portrait-note:overall', 'text')}</section>` : ''}</section>`;
}
...
export function portraitBalanceView(p, s, interlude = '') {
  const m = portraitBalance(p, s.member);
  if (!m) return interlude;
  return `<div class="portrait-balance"><section class="pe-strengths" data-support-topic="balance"><span class="eyebrow">STRENGTHS · WHAT COMES NATURALLY</span>${m.strengths.map((x) => `<article><div class="pe-strength-symbol">${traitGlyph(x.trait)}</div><div><h3>${esc(x.strength)}</h3><p>${esc(x.detail)}</p></div></article>`).join('')}</section>
  ${interlude}
  <section class="pe-other-side" data-support-topic="balance"><span class="eyebrow">POSSIBLE BLIND SPOTS</span><h3>Every strength<br>has another side.</h3><div class="pe-reflection-rule"></div><h4>${esc(m.blind)}</h4><p>${esc(m.caution)}</p><small>A possibility to reflect on, based on ${esc(m.trait.toLowerCase())}. ${m.own ? 'You know what fits.' : 'Only they can say what fits.'}</small></section>
  <section class="pe-question" data-support-topic="balance"><span class="eyebrow">THINGS TO THINK ABOUT</span><blockquote>${esc(m.question)}</blockquote>${m.own ? button('Add your perspective', 'portrait-note:overall', 'text') : ''}</section></div>`;
}

...
function fixedSavingsView(plan) {
  if (!plan) return '';
  return `<section class="savings-maturity" aria-label="Illustrative maturity value">
    <span class="eyebrow">At maturity</span>
    <h3>${conditionDate(plan.maturesOn)}</h3>
    <strong class="maturity-total">${cash(plan.maturityValue)}</strong>
    <p>If you keep to your agreement</p>
    <div class="maturity-breakdown"><div><span>You contribute</span><b>${cash(plan.contributions)}</b></div><div><span>Interest earned</span><b>+${cash(plan.interest)}</b></div></div>
    <div class="maturity-bar" aria-hidden="true"><span style="width:${(plan.contributions / plan.maturityValue) * 100}%"></span></div>
    <details class="maturity-assumptions"><summary>How this is worked out</summary><p>75 month-end payments of ${cash(plan.monthly)} at 5.1% AER fixed, compounded monthly. Interest is held outside the £24,000 balance cap and paid at maturity. Figures are rounded to the nearest pound. Payment timing can change the interest.</p><p>This example plan began on ${conditionDate(plan.startsOn)}. This is a concept illustration, not a product offer.</p></details>
  </section>
  <section class="savings-instalments" aria-label="Instalment progress">
    <div class="instalment-heading"><h3>Every instalment gets you closer.</h3><span><b>${plan.completed}</b> / ${plan.months}</span></div>
    <div class="instalment-grid" role="img" aria-label="${plan.completed} of ${plan.months} instalments completed; ${plan.remaining} remaining. Each dot represents ${cash(plan.monthly)}.">${Array.from({ length: plan.months }, (_, i) => `<span class="${i < plan.completed ? 'is-complete' : i === plan.completed && plan.partial ? 'is-partial' : ''}"></span>`).join('')}</div>
    <div class="instalment-key"><span><i></i>${plan.completed} completed</span><span><i></i>${plan.remaining} remaining</span></div>
    <p>${cash(plan.funded)} contributed so far${plan.partial ? ' · ' + cash(plan.partial) + ' towards the next instalment' : ''}. Each dot is one ${cash(plan.monthly)} instalment.</p>
  </section>`;
}

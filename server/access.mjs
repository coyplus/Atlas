...
function page(path, error = false, status = 200) {
  return new Response(
    `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Welcome to Atlas · HSBC</title><style>*{box-sizing:border-box}body{margin:0;background:#eff3f3;color:#191c1d;font:17px/1.5 -apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;min-height:100dvh;display:grid;place-items:center;padding:28px}main{width:100%;max-width:470px}.brand{display:flex;align-items:center;gap:12px;font-size:25px}.brand span{font-weight:300;border-left:1px solid #bcc7c9;padding-left:14px;color:#647175}svg{width:42px}h1{font-size:clamp(34px,8vw,45px);font-weight:400;line-height:1.1;letter-spacing:-1.5px;margin:56px 0 20px}p{color:#5d6c70;margin:0 0 30px}label{display:block;font-size:15px;margin-bottom:8px}input{font:inherit;width:100%;border:1px solid #aab8bc;border-radius:12px;background:white;padding:16px}input:focus{outline:2px solid #487b93;outline-offset:3px}button{width:100%;padding:16px;border:0;border-radius:12px;background:#191c1d;color:white;font:inherit;margin-top:18px;cursor:pointer}small{display:block;color:#68777b;margin-top:28px;font-size:13px}.error{color:#b00020;font-size:15px;margin:12px 0 0}</style><main><div class="brand"><svg viewBox="0 0 60 30" aria-hidden="true"><path fill="#db0011" d="M0 15 15 0h30l15 15-15 15H15Z"/><path fill="white" d="m15 0 15 15L45 0v30L30 15 15 30Z"/></svg><b>HSBC</b><span>Atlas</span></div><h1>Making banking<br>a relationship again.</h1><p>Enter the password to explore the concept.</p><form method="post" action="/_access"><input type="hidden" name="returnTo" value="${escape(path)}"><label for="password">Password</label><input id="password" name="password" type="password" autocomplete="current-password" required maxlength="200" ${error ? 'aria-invalid="true" aria-describedby="error"' : ''}>${error ? '<p class="error" id="error" role="alert">That password isn’t right. Please try again.</p>' : ''}<button type="submit">Enter prototype</button></form><small>Building better customers builds a better bank.</small></main><script>if(location.hash)document.querySelector('[name=returnTo]').value+=location.hash;</script></html>`,
    { status, headers },
  );
}
